...
// on /api/published and /api/published_data (SEC-01/02/03/04, mirrors
// reearth-visualizer's identical fix). Matches the header name reearth-cloud
// already sends (its classic/server/internal/infrastrucuture/reearth/reearth.go
// setInternalAuth) -- keep these in sync across both repos.
...
// keeps self-hosted deployments (no gateway in front of them, no token ever
