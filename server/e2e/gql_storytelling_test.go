...
func deletePage(e *httpexpect.Expect, sID, storyID, pageID string) (GraphQLRequest, *httpexpect.Value) {
	...
	res.Object().
		...
		Value("story").Object().
		Value("pages").Array().
		Path("$..id").Array().NotContainsAny(pageID)
...
func addLayerToPage(e *httpexpect.Expect, sId, storyId, pageId, layerId string, swipeable *bool) (GraphQLRequest, *httpexpect.Value, string) {
	...
	if swipeable != nil && *swipeable {
		pageRes.Value("swipeableLayers").Array().
			Path("$..id").Array().ContainsAny(layerId)
	} else {
...
func removeBlock(e *httpexpect.Expect, storyID, pageID, blockID string) (GraphQLRequest, *httpexpect.Value, string) {
	...
	blocks := res.Object().Path("$.data.removeStoryBlock.page.blocks").Raw()
	if blocks != nil {
		if blockSlice, ok := blocks.([]interface{}); ok && len(blockSlice) > 0 {
			res.Object().
				Path("$.data.removeStoryBlock.page.blocks[:].id").Array().NotContainsAny(blockID)
		}
	}
...
func moveBlock(e *httpexpect.Expect, storyID, pageID, blockID string, index int) (GraphQLRequest, *httpexpect.Value, string) {
	...
	res.Object().
		Path("$.data.moveStoryBlock.page.blocks[:].id").Array().ContainsAny(blockID)
...
func TestStoryPublishing(t *testing.T) {
	...
	pub := regexp.MustCompile(fmt.Sprintf(
		`(?s)"schemaVersion":1,"id":"%s",".*?"property":\{"tiles":\[\{"id":".*?"\}\]\},"plugins":\{\},"layers":null,"widgets":\[\{"id":".*?","pluginId":"reearth","extensionId":"dataattribution","property":\{\},"extended":false\}\],"widgetAlignSystem":\{"inner":null,"outer":\{"left":\{"top":null,"middle":null,"bottom":\{"widgetIds":\[".*?"\],"align":"start","padding":null,"gap":null,"centered":false,"background":null\}\},"center":null,"right":null\}\},"tags":\[\],"clusters":\[\],"story":\{"id":"%s","property":\{\},"pages":\[\{"id":"%s","property":\{\},"title":"test","blocks":\[\{"id":"%s","property":\{"default":\{"text":"test value"\},"panel":\{"padding":\{"top":2,"bottom":3,"left":0,"right":1\}\}\},"plugins":null,"extensionId":"%s","pluginId":"%s"\}\],"swipeable":true,"swipeableLayers":\[\],"layers":\[\]\}\],"position":"left","bgColor":""\},"nlsLayers":null,"layerStyles":null,"coreSupport":true,"enableGa":false,"trackingId":""`,
		sID, storyID, pageID, blockID, extensionId, pluginId,
	))
