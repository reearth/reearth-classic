...
// route (SEC-02/04): it serves the identical built-scene data /api/published_data
// and /published/:name are gated on, so it must be gated the same way once a
// token is configured -- and left open when one isn't, so self-hosted
...
func TestWeb_DataJSON_RequiresGatewayToken(t *testing.T) {
	...
	newEcho := func(gatewayToken, previousGatewayToken string) *echo.Echo {
		...
		// RequireGatewayToken returns) to the response status correctly.
